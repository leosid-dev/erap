            ...
            for op in operations:
                try:
                    ...
                    if entry in ops_dict[isa][extension][op]:
                      ...
                      #  else:
                      #      op_dict[op][entry]=1
                      #      ext_op_dict[extension][entry]=1
                        # Increment the instruction count for the operation.
                        ops_count[op]['counts'] += n
                        ext_ops_count[extension]['cnt'] += n
                    ...
                    logger.info(f' {entry.instr_name} not found.')
#                    exit(1)

    # Populate the 'Counts' field in the ret_dict with the instruction counts per operation.
    ret_dict['Counts'] = [ops_count[op]['counts'] for op in operations]
    ext_ret_dict['Counts'] = [ext_ops_count[ext]['cnt'] for ext in extension_list]
    del ops_count
    del ext_ops_count
    gc.collect()
